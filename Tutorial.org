...
* The numerical nightmare that is late massive star evolution
...
(where nuclear burning of heavy elements stiffens the equations)
...
We will use a simple model of a 15M_{\odot} star (see [[./15Msun_early_evol/][the work directory]]) to
...
# **** *OPTIONAL*: What about a more physical solution

# MESA already offers options to use \nabla not from MLT but the actual
# temperature gradient without assumption of hydrostatic equilibrium.
# These are available in =controls=: you can try using that without
# =convergence_ignore_equL_residuals=!

# :Hint:
# As usual, search =$MESA_DIR/star/defaults/controls.default= to
# find options available. In this case
# =use_gradT_actual_vs_gradT_MLT_for_T_gradient_eqn= seems relevant! You can
# also find this looking at the =if= statements in
# =$MESA_DIR/star/private/hydro_temperature.f90= which we inspected
# before.
# :end:
