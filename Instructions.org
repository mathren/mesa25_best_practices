...
*N.B.:* Unless the timestep is too small, the initial guess is usually
...
bad derivative w.r.t. the variable =dens= in the last line.
...
*N.B.:* The fact that we ignore the residual in =equL= does not imply this
...
*Bonus task*: you can edit the stopping condition in your =inlist_problem=
to evolve past Oxygen depletion. A second
crash occurs during Si core burning, which is caused by the treatment
of nuclear processes. You can use the things you learned in this lab
to debug this, but remember that the nuclear reaction network we are
using here is insufficient for science focusing on the core of evolved
massive stars!
