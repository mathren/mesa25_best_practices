#+title: Late massive star evolution: physics that challenges the numerics
...
oxygen depletion and call it a success for this mini lab.
...
disk, which should still be =x261=.

*N.B.*: you can use the command line =touch= to update the time of last
edit of a =photo= to trick MESA.
...
checking the definitions in the files in =$MESA_DIR/star_data/public/=.
This is the place where all variables available to MESA are defined.
...
crash. Finally, during these iterations, =lnd= (that is, physically, the
density) is the problematic variable.
...
the luminosity is a solver variable.
...
equation, *not* a luminosity equation!
#+begin_src latex
  \frac{dT}{dm} = \frac{dP}{dm}\frac{T}{P}\nabla \ \ ,
#+end_src
where $T$ is the temperature, $m$ is Lagrangian mass coordinate, $P$
 the pressure and $\nabla=\partial \ln(T)/\partial \ln(P)$ is the temperature gradient
 provided by Mixing Length Theory (MLT). See also [[https://iopscience.iop.org/article/10.1088/0067-0049/192/1/3][Paxton et al. 2011]]
 Sec. 6.2 (specifically Eq. 8) for the discretized formulation of this
 equation.

Why this confusing name then? In a star, the temperature gradient will
adjust to carry the luminosity (leading to convection if the radiative
gradient is insufficient). So we can use the luminosity to calculate
the temperature gradient. However, it is numerically convenient to
flip things, and use the temperature gradient equation to obtain the
...
*Task 1:* Remove the stopping condition for oxygen depletion from your
inlist.

:Hint:
This is set by a pair of =controls= settings.
:end:

:Hint:
Search your =inlist_problem= for =o16=
:end:


** A frustrating (but still common) situation: another problem arise
...
*Task 2*: Turn on the debugging option that revealed the problematic
...
*Task 3*: what residual is problematic? at step =1563=?
...
** TODO Silicon core burning is very hard!
- [[https://iopscience.iop.org/article/10.3847/1538-4365/acae8d][Jermyn et al. 2023]] (Sec. 10) suggests a potential solution =op_split_burn=
- activation temperature issues: we are either below or already above,
  or we have to fine-tune this (not good for a grid!), we will need to
  re-run from scratch. *Fine tuned naturally at* =3e9=.
- Realistically, if we want to reach the onset of core collapse, we
  need to lower =min_timestep_limit=, a lot.
